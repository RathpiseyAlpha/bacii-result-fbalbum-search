...
app.get("/api/archive/:year/documents/:documentId/view", (request, response) => {
  try {
    ...
    const imageUrl = `/api/archive/${year}/documents/${documentId}/pages/${pageNumber}/image?v=2`;
...
<body>
  <header>
    ...
    <div class="header-actions">
      ...
      <!-- Navigation -->
      ${prevPage ? `<a id="prevBtn" class="nav-btn" href="/api/archive/${year}/documents/${documentId}/view?page=${prevPage}">← មុន (Prev)</a>` : ""}
      <span class="page-indicator">ទំព័រ ${pageNumber}</span>
      <a id="nextBtn" class="nav-btn" href="/api/archive/${year}/documents/${documentId}/view?page=${nextPage}">បន្ទាប់ (Next) →</a>
    </div>
  </header>
