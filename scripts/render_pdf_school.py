#!/usr/bin/env python3
"""Render one student's school cell from an official BacII PDF."""
...
def find_row(words: list[tuple], table_number: str) -> list[tuple]:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"Expected one row for table {table_number}, found {len(matches)}."
        )
...
def main() -> None:
    ...
    try:
        ...
        school_words = [word for word in row if 190 <= word[0] < 300 and word[4].strip()]
        if not school_words:
            raise RuntimeError("The matching row has no school text.")

        y0 = max(0, min(word[1] for word in school_words) - 2.5)
        y1 = min(page.rect.height, max(word[3] for word in school_words) + 2.5)
        # The official 2026 result PDFs use x=190..300 for the school column.
        clip = pymupdf.Rect(191, y0, 299, y1)
        ...
        # Remove table rules while preserving the original Khmer glyph pixels.
        with Image.open(temporary) as source:
            ...
            horizontal_rules = [
                ...
                if longest_run(pixels[x, y] < 235 for x in range(image.width))
                > image.width * 0.65
            ]
            vertical_rules = [
                x for x in range(image.width)
                if longest_run(pixels[x, y] < 235 for y in range(image.height))
                > image.height * 0.65
            ]
